...
EOF
  ...
  sudo mv ~/odoo /etc/nginx/sites-available/$WEBSITE_TEST_NAME
  sudo ln -s /etc/nginx/sites-available/$WEBSITE_TEST_NAME /etc/nginx/sites-enabled/$WEBSITE_TEST_NAME
  sudo rm /etc/nginx/sites-enabled/default
  sudo service nginx reload
...
if [ $INSTALL_NGINX = "True" ] && [ $ENABLE_SSL = "True" ] && [ $ADMIN_EMAIL != "odoo@example.com" ]  && [ $WEBSITE_NAME != "_" ];then
  ...
  sudo certbot --nginx -d $WEBSITE_TEST_NAME --noninteractive --agree-tos --email $ADMIN_EMAIL --redirect
  sudo service nginx reload
